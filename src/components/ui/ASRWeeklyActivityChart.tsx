...
export const ASRWeeklyActivityChart = ({ runs, type = "run" }: ActivityChartProps) => {
 ...
 const weekCounts = new Array(52).fill(0).map((_, i) => {
 const diffWeeks = 51 - i;
 ...
 const diffTime = Math.abs(todayStartOfWeek.getTime() - runStartOfWeek.getTime());
 const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
 const diffWeeks = Math.round(diffDays / 7);

 if (runStartOfWeek <= todayStartOfWeek && diffWeeks >= 0 && diffWeeks < 52) {
 weekCounts[51 - diffWeeks].count++;
 runCount++;
 }
 });
 ...
 <div className={cn("w-2 h-2 rounded-[1px]", "bg-black/5 dark:bg-white/5")} title="0" />
 <div className={cn("w-2 h-2 rounded-[1px]", theme === "dark" ? "bg-blue-900/60" : "bg-blue-200")} title="1" />
 <div className={cn("w-2 h-2 rounded-[1px]", theme === "dark" ? "bg-blue-700/80" : "bg-blue-400")} title="2" />
 <div className={cn("w-2 h-2 rounded-[1px]", theme === "dark" ? "bg-blue-500" : "bg-blue-600")} title="3+" />
 </div>
 </div>
 ...
 <div 
 ref={containerRef}
 className="grid grid-cols-[repeat(13,minmax(0,1fr))] gap-1 .5 w-full relative"
 ...
 theme === "dark" ? "hover:ring-blue-400" : "hover:ring-blue-600",
 levelClass
 )}
 onMouseEnter={(e) => {
 ...
 const clampedX = Math.max(75, Math.min(containerRect.width - 75, rawX));
